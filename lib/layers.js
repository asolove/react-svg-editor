...
var ControlLayer = exports.ControlLayer = React.createClass({
	...
	render: function() {
		...
		var controlPoints = controlPointLocations.map(function(location){
			return <rect className="control-point" x={location[0]} y={location[1]} height={controlPointSize} width={controlPointSize}></rect>;
		});

		return <g transform={transformFor(layer.position)} onMouseDown={this.handleMouseDown}>
				  <RectLayer layer={halo}></RectLayer>
				  {controlPoints}
				  <rect className="rotation-line" x={(width-controlLineSize)/2} y={-rotationBarHeight} height={rotationBarHeight} width={controlLineSize}></rect>
				</g>;
	}
});